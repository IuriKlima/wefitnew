...
function createValidEnv() {
  return {
    ...
    ADMIN_ORGANIZATION_ID: "11111111-1111-4111-8111-111111111111",
...
test("rejects an unsafe database connection and inconsistent tenant or JWKS settings", () => {
  const env = {
    ...createValidEnv(),
    DATABASE_URL: "postgresql://runtime:password@database.staging.internal:5432/gym",
    ADMIN_ORGANIZATION_ID: "not-a-uuid",
    ADMIN_UNIT_ID: "not-a-uuid",
  ...
  assert.deepEqual(validateReleaseConfig(env), [
    ...
    "ADMIN_ORGANIZATION_ID must be a UUID.",
    "ADMIN_UNIT_ID must be a UUID when configured.",
