...
Use ainda `SWAGGER_ENABLED=false`, `NODE_ENV=production`, `AUTH_ADAPTER=supabase-jwt`, `ADMIN_AUTH_ADAPTER=supabase-jwt`, `ORGANIZATION_SELF_SERVICE_ENABLED=false`, `NEXT_PUBLIC_ORGANIZATION_SELF_SERVICE_ENABLED=false` e um `RATE_LIMIT_MAX` dimensionado. Nao disponibilize `x-dev-user-id`, `temporary-header`, `ADMIN_ORGANIZATION_ID` ou `ADMIN_UNIT_ID` em staging/producao. O onboarding self-service permanece fechado; as duas aplicacoes rejeitam a flag ativa em producao.
...
As migrations de contexto criam uma funcao privilegiada estreita. Antes de subir a API, conceda a role real de runtime somente as memberships `wefit_context_consumer` e `wefit_onboarding_consumer`. Nunca conceda `wefit_context_reader` ou `wefit_onboarding_owner` ao runtime. A inicializacao da API deve confirmar zero para superuser, `BYPASSRLS`, heranca elevada e ownership de tabelas de negocio.

## Rollout do onboarding

O procedimento detalhado esta no [Runbook do onboarding guiado](./onboarding-runbook.md).

- Desenvolvimento e testes podem habilitar as duas flags explicitamente para validar a jornada.
- Staging e producao ficam com as flags desligadas nesta versao; nenhum cadastro deve ser aberto por
  alteracao emergencial de ambiente.
- A liberacao futura exige nova decisao aprovada, textos legais definitivos, monitoramento, suporte,
  teste com JWT Supabase real e verificacao da role restrita.
- A selecao de plano no fluxo e apenas configuracao de produto; nao cria assinatura, cobranca,
  pagamento ou direito comercial.
...
5. Criar o usuario no Supabase e provisionar o primeiro tenant conforme [Provisionamento do beta fechado](./closed-beta-provisioning.md). O onboarding guiado pode ser exercitado em ambiente local/teste isolado, mas nao substitui o provisionamento controlado do beta.
...
- As migrations novas foram validadas somente no PostgreSQL 17 de teste local; nao foram aplicadas a staging ou producao. As funcoes, RLS, `wefit_context_consumer`, `wefit_onboarding_consumer` e grants precisam ser validados com a role real do staging.
...
- A fronteira transacional tenant-aware cobre contexto, unidades, alunos e onboarding. O self-service
  permanece fechado em producao.
- As paginas legais do cadastro sao preliminares e precisam de aprovacao juridica antes de qualquer
  abertura publica.
